...
export const cvPortlandDefaultOrder: string[] = [
  "identitySection",
  "personalInfoSection",
  "contactSection",
  "profileSection",
...
export const CvPortland: React.FC<ITemplateProps> = (props) => {
  ...
  // PREPARAR SECCIONES
  // ---------------------------------------------------------------------------
  const portlandSections = React.useMemo(() => {
  ...
  const headerSection = portlandSections.filter((s) => s.orientation === "header");
...
const [page2Vertical, setPage2Vertical] = React.useState<string[]>([]);
  ...
  const measure = () => {
    ...
    const nodes = Array.from(
      container.querySelectorAll(".cv-portland__vertical > div")
    );
    ...
    nodes.forEach((node) => {
      ...
      const sectionName =
        node.className.match(/cv-portland__(.*?)\s/)?.[1] ?? "unknown";
  ...
  return (
    <div className="cv-portland" style={{ fontFamily: styles.fontFamily }}>
      ...
      <div className="cv-portland__hidden--vertical" ref={verticalContainerRef}>
        <div className="cv-portland__limit-line"/>
        <div className="cv-portland__limit-line-overflowed"/>
        {headerSection
         .map((s) => (
          <div onClick={()=>handleClick(s.name)} className={`cv-portland__${s.name} ${isEditorOpen(s.name) ? "cv-portland__section--active" : "cv-portland__section"} cv-portland__section--header`}>
            {s.render}
          </div>
        ))}

        <div className="cv-portland__vertical">
            {portlandSections
              .filter((s) => s.name !== "identitySection")
              .map((s) => (
                <div onClick={()=>handleClick(s.name)} className={`cv-portland__${s.name} ${isEditorOpen(s.name) ? "cv-portland__section--active" : "cv-portland__section"} cv-portland__section--vertical`}>
                  {s.render}
                </div>
              ))}
          </div>
      ...
      <div className="cv-portland__page">
        ...
        {headerSection.map((s) => (
          <div
            ...
            className={`cv-portland__${s.name} ${
              isEditorOpen(s.name) ? "cv-portland__section--active" : "cv-portland__section"
            } cv-portland__section--header`}
          >
            {s.render}
          </div>
        ))}
        ...
        <div className="cv-portland__vertical">
            {page1Vertical.map((name) => {
              const sec = portlandSections.find((s) => s.name === name);
              if (!sec) return null;
              return (
                <div
                  key={name}
                  onClick={() => handleClick(name)}
                  className={`cv-portland__${name} ${
                    isEditorOpen(name)
                      ? "cv-portland__section--active"
                      : "cv-portland__section"
                  } cv-portland__section--vertical`}
                >
                  {sec.render}
                </div>
              );
        ...
        {(page2Vertical.length > 0) && (
          <>
          <p className="cv-portland__page--number">Pagina 1</p>
          <div className="cv-portland__limit-page">{previewPopupOpen === false && sidebarOption === "create" && <p className="cv-portland__limit-page-text">Pagina 2</p>} </div>
         </>
      ) }
      </div>
      ...
      {(page2Vertical.length > 0) && (
        <div className="cv-portland__page">
          ...
          <div className="cv-portland__limit-line-overflowed">
            {previewPopupOpen === false && sidebarOption === "create" && <p className="cv-portland__limit-line-overflowed-text"><LuTriangleAlert /> Los reclutadores leen primero lo esencial: mantén tu CV en máximo 2 páginas.</p>}
          </div>
          <div className="cv-portland__vertical">
          ...
          <p className="cv-portland__page--number">Pagina 2</p>
        </div>
      )}
      </div>
