...
export const cvParisDefaultOrder: string[] = [
  "identitySection",
  "personalInfoSection",
  "contactSection",
  "profileSection",
...
export const CvParis: React.FC<ITemplateProps> = (props) => {
  ...
  // PREPARAR SECCIONES
  // ---------------------------------------------------------------------------
  const parisSections = React.useMemo(() => {
  ...
  const verticalSections = parisSections.filter((s) => s.orientation === "vertical");
  const horizontalSections = parisSections.filter((s) => s.orientation === "horizontal");
  const headerSection = parisSections.filter((s) => s.orientation === "header");
...
const [page2Horizontal, setPage2Horizontal] = React.useState<string[]>([]);
  ...
  const measure = () => {
    ...
    const nodes = Array.from(
      container.querySelectorAll(".cv-paris__split--vertical > div")
    );
    ...
    nodes.forEach((node) => {
      ...
      const sectionName =
        node.className.match(/cv-paris__(.*?)\s/)?.[1] ?? "unknown";
...
useEffect(() => {
  ...
  const measure = () => {
    ...
    const nodes = Array.from(
      container.querySelectorAll(".cv-paris__split--horizontal > div")
    );
  ...
  return (
    <div className="cv-paris" style={{ fontFamily: styles.font }}>
      ...
      <div className="cv-paris__hidden--vertical" ref={verticalContainerRef}>
        <div className="cv-paris__limit-line"/>
        <div className="cv-paris__limit-line-overflowed"/>
        {headerSection
         .map((s) => (
          <div onClick={()=>handleClick(s.name)} className={`cv-paris__${s.name} ${isEditorOpen(s.name) ? "cv-paris__section--active" : "cv-paris__section"} cv-paris__section--header`}>
            {s.render}
          </div>
        ))}

        <div className="cv-paris__split">
          <div className="cv-paris__split--vertical paris-page-one">
            {verticalSections
              .filter((s) => s.name !== "identitySection")
              .map((s) => (
                <div onClick={()=>handleClick(s.name)} className={`cv-paris__${s.name} ${isEditorOpen(s.name) ? "cv-paris__section--active" : "cv-paris__section"} cv-paris__section--vertical`}>
                  {s.render}
                </div>
              ))}
          </div>

          <div className="cv-paris__split--horizontal paris-page-one">
          </div>
        </div>
      </div>
      {/* medicion oculta por overflow columna horizontal */}
      <div className="cv-paris__hidden--horizontal" ref={horizontalContainerRef}>
        <div className="cv-paris__limit-line"/>
        <div className="cv-paris__limit-line-overflowed"/>
        {headerSection
         .map((s) => (
        ...
        <div className="cv-paris__split">
          ...
          <div className="cv-paris__split--horizontal paris-page-one">
            {horizontalSections
              .filter((s) => s.name !== "identitySection")
              .map((s) => (
                <div onClick={()=>handleClick(s.name)} className={`cv-paris__${s.name} ${isEditorOpen(s.name) ? "cv-paris__section--active" : "cv-paris__section"} cv-paris__section--horizontal`}>
                  {s.render}
                </div>
              ))}
          </div>
      ...
      <div className="cv-paris__page">
        <div className="cv-paris__page--bg-top"></div>
        <div className="cv-paris__page--bg-bottom"></div>
        ...
        {headerSection.map((s) => (
          <div
            ...
            className={`cv-paris__${s.name} ${
              isEditorOpen(s.name) ? "cv-paris__section--active" : "cv-paris__section"
            } cv-paris__section--header`}
          >
            {s.render}
          </div>
        ))}
        ...
        <div className="cv-paris__split">
          <div className="cv-paris__split--vertical paris-page-one">
            {page1Vertical.map((name) => {
              const sec = parisSections.find((s) => s.name === name);
              if (!sec) return null;
              return (
                <div
                  key={name}
                  onClick={() => handleClick(name)}
                  className={`cv-paris__${name} ${
                    isEditorOpen(name)
                      ? "cv-paris__section--active"
                      : "cv-paris__section"
                  } cv-paris__section--vertical`}
                >
                  {sec.render}
                </div>
              );
        ...
        {(page2Vertical.length > 0 || page2Horizontal.length > 0) && (
          <>
          <p className="cv-paris__page--number">Pagina 1</p>
          <div className="cv-paris__limit-page">{previewPopupOpen === false && sidebarOption === "create" &&  <p className="cv-paris__limit-page-text">Pagina 2</p>} </div>
         </>
      ) }
      </div>
      ...
      {(page2Vertical.length > 0 || page2Horizontal.length > 0) && (
        ...
        {(!hasValidSubscriptionTime(subscriptionExpiresAt) && sidebarOption !== "home" && sidebarOption !== "cv") && (
          ...
          <div className="cv-paris__limit-line-overflowed">
            {previewPopupOpen === false && sidebarOption === "create" && <p className="cv-paris__limit-line-overflowed-text"><LuTriangleAlert /> Los reclutadores leen primero lo esencial: mantén tu CV en máximo 2 páginas.</p>}
          </div>
          <div className="cv-paris__split">
            <div className="cv-paris__split--vertical">
            ...
            <div className="cv-paris__split--horizontal">
          ...
          <p className="cv-paris__page--number">Pagina 2</p>
        </div>
      )}
      </div>
