...
export const CvTokyoHeader = React.forwardRef<HTMLDivElement, HeaderProps>(
  (props, ref) => {
    const section = useSelector((state: IState) =>
      state.cvSections.sections.find((s) => s.name === "identitySection")
    );

    const { qrCodeUrl, allowQrCode } = useSelector(
    ...
    const { fullName, occupation, photo, allowCvPhoto, styles } = props;
      ...
      >
        {!previewPopupOpen &&
          typeof section?.progress === "number" && (
            <span
              className={`progress-indicator cv-tokyo-header__progress-indicator ${getProgressColorClass(
                section.progress
              )}`}
            >
              {section.progress}%
        ...
        <div className="cv-tokyo-header__main">
          ...
          <div className="cv-tokyo-header__identitySection--text">
            <h1
              className="cv-tokyo-header__identitySection--title"
              style={{ color: styles.title }}
            >
              {fullName}
            </h1>
            <p
              className="cv-tokyo-header__identitySection--occupation"
        ...
        {allowQrCode && (
          <div className="cv-tokyo-header__identitySection--qr-wrapper">
            <QRCodeSVG
              ...
              fgColor={styles.qr}
            />
            <p className="cv-tokyo-header__identitySection--qr-text">
              Ver CV Online
            </p>
          </div>
        )}
